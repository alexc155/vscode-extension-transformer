...
import { Region, View } from '../common/EditorFunctions';
...
const DEFAULT_SCRIPT =
  '// - This Feature is BETA, scripts may not work in future versions\n' +
  '// - macro commands:\n' +
  '//   t - "text to type"\n' +
  '//   c - cursor commands\n' +
...
vscode.languages.registerCodeLensProvider(
  { language: 'plaintext' },
  {
    provideCodeLenses: (document, cancel) => {
      ...
      return [
        View.makeCodeLens('Set current text as starting point for macro test', 0, 0, updatePreviewTextInitialState),
      ];
    },
...
async function evalFunctionExpressionAsPreview(expression: string, previewEditor: vscode.TextEditor) {
  ...
  previewEditor.setDecorations(cursorDecorationType, [
    View.createCursorDecorator(previewEditor.selection.anchor.line, previewEditor.selection.anchor.character),
  ]);
...
export function runCurrentMacro() {
  if (vscode.window.activeTextEditor.selection.isEmpty) return evalFunctionExpression(currentMacroScript.content);
  else return runCurrentMacroEachLine(vscode.window.activeTextEditor.selection);
}
...
export async function deleteMacro() {
  ...
  if (answer === 'Delete') await MacroRepository.deleteMacro(selectedItem.label);
}
...
async function openMacroEditor() {
  ...
  scriptWatcher = View.watchDocument(scriptEditor.document, async (event) => {
    ...
    if (event.eventType === 'selection' && event.cursorMoved === 'vertical') {
      ...
      const scriptContentRange = Region.makeRangeFromStartToLine(
        scriptEditor.document,
        scriptEditor.selection.end.line
      );
