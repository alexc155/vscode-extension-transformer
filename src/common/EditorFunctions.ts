...
export namespace Region {
  export function makeRangeFromFoldingRegion(
    document: vscode.TextDocument,
    foldableLineNumber: number,
    tabSize: number
  ) {
    ...
    if (!endFoldLine) {
      ...
      if (endLineNumber > foldableLineNumber) foldableLineNumber++;
    }
   ...
   */
  export function makeRangeFromFoldingRegionRelativeLevel(
    document: vscode.TextDocument,
    lineNumber: number,
  ...
  export function makeRangeDocument(document: vscode.TextDocument) {
    ...
    return new vscode.Range(
      ...
      new vscode.Position(rangeLastLine.line, rangeLastLine.character)
    );
  }
  ...
  export function expandRangeToBlockIfEmpty(textEditor: vscode.TextEditor, range: vscode.Range) {
    if (range.isSingleLine && range.start.character === range.end.character) {
      const firstLineOfBlock = Lines.findFirstLineOfBlock(
        textEditor.document,
        range.start.line,
        (line) => !line.isEmptyOrWhitespace
      );
      const lastLineOfBlock = Lines.findLastLineOfBlock(
        textEditor.document,
        range.start.line,
        (line) => !line.isEmptyOrWhitespace
      );
      return new vscode.Range(
        new vscode.Position(firstLineOfBlock.lineNumber, 0),
        new vscode.Position(lastLineOfBlock.lineNumber, lastLineOfBlock.range.end.character)
      );
    }
    return range;
  }
  ...
  export function expandRangeToLineIfEmpty(document: vscode.TextDocument, range: vscode.Range) {
    if (range.isEmpty) return expandRangeFullLineWidth(document, range);
    return range;
  }
  ...
  export function makeVerticalRangesWithinBlock(textEditor: vscode.TextEditor, ranges: vscode.Range[]) {
    ...
    if (range.isSingleLine && range.start.character === range.end.character) {
      ...
      const cursorColumn = Lines.calculateColumnFromCharIndex(
  ...
  export function makeSelectionsFromRanges(ranges: vscode.Range[]) {
    return ranges.map(
      (range) => new vscode.Selection(range.start.line, range.start.character, range.start.line, range.start.character)
    );
  }
  ...
  export function findAllRangesContaining(document: vscode.TextDocument, text: string) {
    ...
    for (let index = 0; index < document.lineCount; index++) {
      ...
      if (indexMatch > -1) {
        ranges.push(
          new vscode.Range(
            new vscode.Position(line.lineNumber, indexMatch),
            new vscode.Position(line.lineNumber, indexMatch + text.length)
          )
        );
      }
    }
   ...
   * @returns
   */
  export function contentChangesInVisibleRanges(
    contentChanges: readonly vscode.TextDocumentContentChangeEvent[],
    visibleRanges: vscode.Range[]
  ) {
    const visibleChanges = [] as vscode.TextDocumentContentChangeEvent[];
    for (const change of contentChanges) {
      if (visibleRanges.find((range) => range.contains(change.range))) visibleChanges.push(change);
    }
...
export namespace Modify {
  export function replaceLinesWithText(
    textEditor: vscode.TextEditor,
    linesOld: Array<vscode.TextLine>,
    linesNew: Array<string>
  ) {
  ...
  export function replaceRangesWithText(
    textEditor: vscode.TextEditor,
    rangesOld: Array<vscode.Range>,
    textsNew: Array<string>
  ) {
    textEditor.edit(function (editBuilder) {
      let lineIndex = 0;
  ...
  export function reverseLines(textEditor: vscode.TextEditor, ranges: Array<vscode.Range>) {
    if (ranges.length === 1) {
      const linesToReverse = Lines.linesFromRange(
        textEditor.document,
        Region.expandRangeToBlockIfEmpty(textEditor, ranges[0])
      );
  ...
  export function replaceLines(
    textEditor: vscode.TextEditor,
    linesOld: Array<vscode.TextLine>,
    linesNew: Array<vscode.TextLine>
  ) {
    replaceLinesWithText(
      textEditor,
      linesOld,
      linesNew.map((line) => line.text)
    );
  }
  ...
  export function replaceRanges(
    textEditor: vscode.TextEditor,
    rangesOld: Array<vscode.Range>,
    rangesNew: Array<vscode.Range>
  ) {
    replaceRangesWithText(
      textEditor,
      rangesOld,
      rangesNew.map((range) => textEditor.document.getText(range))
    );
  }
  ...
  export function sortLinesByColumn(textEditor: vscode.TextEditor, ranges: Array<vscode.Range>) {
    ...
    const sortedLines = orderByNatural(
      lines,
      [(line) => Region.textFromRangeOrCursorToEndLine(line.line.text, line.range)],
      null
    );
    ...
    replaceLines(
      ...
      sortedLines.map((line) => line.line)
    );
    // when we sort the lines, selections are not moved with the lines
    // we update the selections here to match the moved lines
    const updatedRanges = makeRangesFromCombined(
      textEditor,
      lines.map((line) => line.range),
      sortedLines.map((line) => line.range)
    );
   ...
   */
  function makeRangesFromCombined(
    textEditor: vscode.TextEditor,
    rangesLinesSource: Array<vscode.Range>,
    rangesCharPosSource: Array<vscode.Range>
  ) {
    const newRanges = [];
    let lineIndex = 0;
    rangesLinesSource.forEach((rangeLineSource) => {
      ...
      newRanges.push(
        new vscode.Range(
          new vscode.Position(rangeLineSource.start.line, ranchCharSource.start.character),
          new vscode.Position(rangeLineSource.end.line, ranchCharSource.end.character)
        )
      );
      lineIndex++;
    });
  ...
  export function add(textEditor: vscode.TextEditor, position: vscode.Position, blockText: string) {
    textEditor.edit(function (editBuilder) {
      editBuilder.insert(position, blockText);
    });
  }

  export function replaceUsingTransform(
    textEditor: vscode.TextEditor,
    ranges: vscode.Range[],
...
export namespace View {
  ...
  export function createGutterDecorator(
    lineNumber: number,
    contentText: string,
    width: string
  ...
  export function setLineDecorators(
  ...
  export function addMarkdownCommandLink(
  ...
  export class QuickPickActionableReactive implements QuickPickActionable {
    ...
    set value(value) {
      this._value = value;
      if (value instanceof QuickPickActionableReactive) this.detail = 'value: ' + value.label;
      else this.detail = 'value: ' + this._value;
    }
  }
  ...
  export function promptOptions(
    items: QuickPickActionable[],
    onChange?: (item: QuickPickActionable, action: QuickPickActionType) => any
  ): Thenable<QuickPickActionable | void> {
    ...
    return vscode.window.showQuickPick(items, { ignoreFocusOut: true }).then((item) => {
      ...
      if (item.children) {
        ...
        if (typeof item.children === 'function') resolveChildren = item.children();
        else resolveChildren = Promise.resolve(item.children as QuickPickActionable[]); // convert value to promise
        ...
        item.input.validateInput = (input) => {
          if (item.value != input) {
            ...
            if (onChange) return onChange(item, QuickPickActionType.INPUT);
          }
          return null;
        };
  ...
  export async function openShowDocument(name: string, content: string, preserveFocus = true, viewColumn?: number) {
    ...
    if (!document) {
      ...
      if (!editor) editor = await vscode.window.showTextDocument(document, viewColumn, preserveFocus);
    }
  ...
  export async function liveDocumentView(documentName: string, initialContent: string, viewRenderer: LiveViewRenderer) {
    ...
    const viewEditor = await openShowDocument(
      'Macro Preview.' + Application.documentExtension(lastActiveSourceDocument),
      '',
      false
    );
  ...
  export function addTreeItem(
    parent: TreeItemActionable,
    item: TreeItemActionable,
    atPosition?: ((children: TreeItemActionable[]) => number) | number
  ) {
    item.parent = parent;
    ...
    } else parent.children.push(item); // default to insert last
    return item;
  }

  export function makeTreeViewManager(
    context: vscode.ExtensionContext,
    viewId: string,
    rootTreeItem?: TreeItemActionable
  ): ITreeViewManager {
    ...
    const provider = {
      ...
      getChildren: (element) => {
        ...
        if (element) children = treeItemActionable.children;

        if (!children) return;

        if (children instanceof Function) return Promise.resolve(children());
        else return Promise.resolve(children);
      },
    ...
    function removeTreeItems(
      parent: TreeItemActionable,
      shouldRemove: (treeItem: TreeItemActionable, index: number) => boolean
    ) {
  ...
  export function registerIcons(context: vscode.ExtensionContext, basepath: string) {
    ...
    paths
      ...
      .forEach((path) => {
        if (path.endsWith('.svg'))
          ICON_PATHS.set(
            path.substring(0, path.length - 4) + '.light',
            context.asAbsolutePath(Path.join(basepath, 'light', path))
          );
      });
    ...
    paths
      ...
      .forEach((path) => {
        if (path.endsWith('.svg'))
          ICON_PATHS.set(
            path.substring(0, path.length - 4) + '.dark',
            context.asAbsolutePath(Path.join(basepath, 'dark', path))
          );
      });
  }
...
export namespace Application {
  ...
  export function registerInternalCommandProxy(context: vscode.ExtensionContext) {
    let disposable = vscode.commands.registerCommand('alexc155-internal.oncommand', (onCommand: Function, ...params) =>
      onCommand.apply(this, params)
    );
